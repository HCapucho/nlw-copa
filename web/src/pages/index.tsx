...
export default function Home(props: HomeProps) {
  ...
  return (
    <div className="max-w-[1124px] h-screen mx-auto grid grid-cols-2 gap-28 items-center">
      <main>
        ...
        <form className='mt-10 flex gap-2' onSubmit={createPool}>
          <input className='flex-1 px-6 py-4 rounded bg-gray-800 border border-gray-600 text-sm text-gray-100'
            ...
            value={poolTitle}
            required />
          <button className='bg-yellow-500 hover:bg-yellow-700 px-6 py-4 rounded text-gray-900 font-bold text-sm uppercase' type="submit">Criar meu bolão</button>
        </form>
      ...
      <Image src={appPreviewImg} quality={100} alt="Dois celulares exibindo uma prévia da aplicação móvel do NLW Copa" />
    </div>
  )
}
